...
if ( ! class_exists( 'Alg_WC_Payment_Gateways_By_User_Roles' ) ) :
	...
	final class Alg_WC_Payment_Gateways_By_User_Roles {
		...
		/**
		 * Settings tab for wooCommerce setting.
		 * @var string $settings
		 */
		public $settings = '';
		...
		public function admin() {
			//HPOS compatibilty
			add_action( 'before_woocommerce_init', array( $this, 'pgur_declare_hpos_compatibility' ) );
			// Action links.
			...
			add_action( 'init', array( $this, 'pgur_load_general_settings' ), 20 );
			$this->settings            = array();
		...
		function pgur_load_general_settings() {
			require_once 'includes/settings/class-alg-wc-payment-gateways-by-user-roles-settings-general.php';
			Alg_WC_Payment_Gateways_By_User_Roles::instance()->settings['general'] = new Alg_WC_Payment_Gateways_By_User_Roles_Settings_General();
		}
		 ...
		 * @return void
		 */
		public function pgur_declare_hpos_compatibility() {
